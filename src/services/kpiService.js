...
// Get all KPI records (admin) — MANDATORY company scope
export async function getAllKPI(companyId) {
  if (!guardCompanyId(companyId, 'getAllKPI')) return { data: [], error: null };
...
// Create KPI record with metrics — includes company_id
export async function createKPI(employeeId, period, overallScore, metrics, companyId) {
  // Insert record with company_id for proper tenant scoping
  const insertPayload = { employee_id: employeeId, period, overall_score: overallScore };
...
export const getAllKPIs = getAllKPI;
