...
import { getDefaultAvatar, AVATARS } from '../lib/avatarConfig';
...
export default function EmployeeModal({ isOpen, onClose, onSave, employee = null, divisions = [] }) {
 ...
 const [previewAvatar, setPreviewAvatar] = useState(() => {
   // If editing existing employee with photo, show it; else pick a default
   if (employee?.photo_url) return employee.photo_url;
   return getDefaultAvatar(employee?.gender || '');
 });

 const refreshAvatar = (gender) => {
   const g = (gender || '').toLowerCase();
   const pool = (g === 'female' || g === 'perempuan') ? AVATARS.female
     : (g === 'male' || g === 'laki-laki') ? AVATARS.male
     : AVATARS.all;
 ...
 // Reset form when employee prop changes (e.g., editing a different employee)
 useEffect(() => {
 if (isOpen) {
 setForm(employee ? {
 name: employee.name || '', email: employee.email || '', phone: employee.phone || '',
 ...
 setPreviewAvatar(employee?.photo_url || getDefaultAvatar(employee?.gender || ''));
 ...
 {/* Avatar Preview */}
 <div className="form-group" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
   <label className="form-label">Avatar Otomatis</label>
   <div style={{ position: 'relative', width: 72, height: 72 }}>
     <img
       src={previewAvatar}
       alt="Avatar preview"
       style={{ width: 72, height: 72, borderRadius: 16, border: '2px solid var(--border)', objectFit: 'cover' }}
     />
     <button
       type="button"
       onClick={() => refreshAvatar(form.gender)}
       title="Ganti avatar"
       style={{
   ...
   <span style={{ fontSize: 11, color: 'var(--muted)', textAlign: 'center' }}>Pilih gender utk avatar sesuai</span>
 </div>
