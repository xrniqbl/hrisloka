...
import { TableSkeleton, PageSkeleton } from '../components/SkeletonLoader';
import EmptyState from '../components/EmptyState';
import BranchFilter from '../components/BranchFilter';
import { useBranch } from '../context/BranchContext';
import { useRealtimeTable } from '../hooks/useRealtime';
import { sendWhatsApp, buildLeaveStatusMessage } from '../services/whatsappService';
import { sendLeaveApprovalEmail } from '../services/emailService';
import { sendLeavePush } from '../services/pushService';
...
export default function LeaveManagement() {
  ...
  }, [selectedBranchId, employee?.company_id]);

  // Realtime: auto-refresh on leave_requests changes
  ...
  const handleApprove = async (id) => {
    ...
    if (!error) {
      ...
      if (leave) {
        ...
        if (emp) {
          const leaveType = leave.type === 'cuti' ? 'Cuti Tahunan' : leave.type === 'sakit' ? 'Cuti Sakit' : 'Izin';
          const startDate = new Date(leave.start_date || leave.startDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
          const endDate = new Date(leave.end_date || leave.endDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
          if (emp.phone) sendWhatsApp(emp.phone, buildLeaveStatusMessage(emp.name, 'approved', leaveType, startDate, endDate));
          if (emp.email) sendLeaveApprovalEmail({ employeeName: emp.name, employeeEmail: emp.email, status: 'approved', leaveType, startDate, endDate });
          sendLeavePush(leave.employee_id, 'approved', leaveType).catch(() => {});
        }
      }
    }
  };
    ...
    if (!error) {
      ...
      if (leave) {
        ...
        if (emp) {
          ...
          const startDate = new Date(leave.start_date || leave.startDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
          const endDate = new Date(leave.end_date || leave.endDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
          if (emp.phone) sendWhatsApp(emp.phone, buildLeaveStatusMessage(emp.name, 'rejected', leaveType, startDate, endDate));
          if (emp.email) sendLeaveApprovalEmail({ employeeName: emp.name, employeeEmail: emp.email, status: 'rejected', leaveType, startDate, endDate });
          sendLeavePush(leave.employee_id, 'rejected', leaveType).catch(() => {});
        }
      }
    }
  };
                  ...
                  return (
                    <tr key={leave.id}>
                      ...
                      <td><span className={`status-badge ${leave.status}`}>
                        {leave.status === 'pending' ? 'Pending' : leave.status === 'approved' ? 'Approved' : leave.status === 'cancelled' ? 'Dibatalkan' : 'Rejected'}
                      </span></td>
                      <td>
