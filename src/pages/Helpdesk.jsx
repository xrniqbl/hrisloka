...
function getSlaStatus(ticket) {
  ...
  const created = new Date(ticket.created_at || ticket.createdDate);
  const now = new Date();
  const hoursElapsed = (now - created) / (1000 * 60 * 60);
  const slaHours = ticket.sla_hours || ticket.slaHours || 24;
...
export default function Helpdesk() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const { data: tktData } = await ticketService.getAllTickets(employee?.company_id);
  ...
  useRealtimeTable('tickets', fetchData);
  ...
  const filtered = ticketList.filter(t => {
    ...
    const matchSearch = (t.subject || '').toLowerCase().includes(search.toLowerCase()) || String(t.id).toLowerCase().includes(search.toLowerCase());
  ...
  const handleSubmit = async () => {
    const { error } = await ticketService.submitTicket({
  ...
  const handleStatusChange = async (id, newStatus) => {
    const { error } = await ticketService.updateTicket(id, { status: newStatus });
    if (!error) {
      fetchData();
  ...
  if (loading) return <PageSkeleton hasStats={false} tableRows={6} tableCols={5} />;
  return (
    <div>
            ...
            {filtered.map(t => {
              ...
              return (
                <tr key={t.id}>
                  ...
                  <td><span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600, color: '#fff', background: priorityColors[t.priority] || priorityColors.medium }}>{t.priority ? t.priority.charAt(0).toUpperCase() + t.priority.slice(1) : 'Medium'}</span></td>
                  <td><span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600, color: '#fff', background: statusColors[t.status] || statusColors.open }}>{statusLabels[t.status] || t.status}</span></td>
            ...
            <div className="modal-body">
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <span style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, color: '#fff', background: priorityColors[selected.priority], display: 'inline-block' }}>{selected.priority ? selected.priority.charAt(0).toUpperCase() + selected.priority.slice(1) : 'Medium'}</span>
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{selected.sla_hours || selected.slaHours || 24} jam</div>
                </div>
                <div className="form-group">
                ...
                <div className="form-group full-width">
                  ...
                  <div style={{ fontSize: 14, lineHeight: 1.6, padding: 12, background: 'var(--bg)', borderRadius: 'var(--radius-sm)' }}>{selected.description || selected.message}</div>
                </div>
              </div>
