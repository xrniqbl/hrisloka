...
export async function generatePayslipPDF(employee, payrollData) {
    ...
    const deductionItems = [
        ['BPJS Kesehatan', Math.round((payrollData.baseSalary || 0) * 0.01)],
        ['BPJS JHT (2%)', Math.round((payrollData.baseSalary || 0) * 0.02)],
        ['BPJS JP (1%)', Math.round((payrollData.baseSalary || 0) * 0.01)],
        ['PPh 21', payrollData.tax || 0],
    ];
    ...
    const grossIncome = (payrollData.baseSalary || 0) + (payrollData.allowance || 0) + (payrollData.overtimePay || 0);
    const totalBPJS = Math.round((payrollData.baseSalary || 0) * 0.04);
    const totalDeductions = totalBPJS + (payrollData.tax || 0);
    ...
    doc.setFontSize(7);
    doc.text('Dokumen ini dibuat secara otomatis oleh sistem HRISync dan tidak memerlukan tanda tangan.', W / 2, y, { align: 'center' });
